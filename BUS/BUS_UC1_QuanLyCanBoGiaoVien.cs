...
namespace BUS
{
    public class BUS_UC1_QuanLyCanBoGiaoVien
    {
        ...
        {
            string sql = "select * from CanBoGiaoVien where MaCanBoGV like N'%" + dk + "%' OR HoTenGV like N'%" + dk + "%' OR DiaChi like N'%" + dk + "%' OR TaiKhoan like N'%" + dk + "%' OR LoaiTaiKhoan like '%" + dk + "%' OR MaTD like N'%" + dk + "%'";
            DataTable dt = new DataTable();
            dt = da.GetTable(sql);
            return dt;
        }
